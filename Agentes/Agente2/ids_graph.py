...
class GraphAgent:
    ...
    async def ainvoke(self, role : str, log):
        if self.agent == None:
        ...
        return result[MESSAGES][-1] # Devolvemos el ultimo mensaje generado por el agente
    ...
    # Nodo de entrada, carga las tools disponibles
    async def node_load_tools(self, state : AgentState):
        print("Entering NODE: node_load_tools")
        tools = await self.get_mcp_tools()
        #agents = await self.get_a2a_agents()
        #print(len(agents))
        return {
            **state,
            TOOLS : tools
    ...
    # Nodo encargado de cargar los agentes
    async def node_load_agents(self, state : AgentState):
        print("Entering NODE: node_load_agents")
        agents = await self.get_a2a_agents()
        return {
            **state,
            AGENTS : agents
    ...
    async def node_execute_selected_tool(self, state : AgentState):
    ...
    async def node_retrieve_information(self, state : AgentState):
        ...
        if tools is None or tools == []:
            ...
            decision = self.llmController.completion(prompt)
            parsed = {}
            try:
    ...
    async def node_evaluator_phase1(self, state: AgentState):
        ...
        response = self.llmController.completion(prompt)
        parsed = {}
        try:
    ...
    async def node_evaluator_phase2(self, state: AgentState):
    ...
    async def node_select_agent(self, state: AgentState):
    ...
    async def node_comunicate_a2a(self, state: AgentState):
    ...
    def router_retriever(self, state : AgentState):
    ...
    def router_phase1(self, state : AgentState):
    ...
    def router_comunicate_a2a(self, state: AgentState):
        role = state[MESSAGES][-1].get("role")
        if role == ROLE_NODE_COMUNICATE_A2A_SUCCESS:
            return NAME_NODE_EVALUATOR_PHASE2
        return NAME_NODE_FINAL_ANSWER

    def router_load_tools(self, state: AgentState):
        tools = state.get(TOOLS, None)
        if tools == None or tools == []:
            return NAME_NODE_EVALUATOR_PHASE1
        return NAME_NODE_RETRIEVE_INFORMATION
    
    def router_load_agents(self, state: AgentState):
    ...
    def makeGraph(self) -> StateGraph:
        graph = StateGraph(AgentState)

        graph.add_node(NAME_NODE_LOAD_TOOLS, self.node_load_tools)
        graph.add_node(NAME_NODE_RETRIEVE_INFORMATION, self.node_retrieve_information)
        graph.add_node(NAME_NODE_EXECUTE_SELECTED_TOOL, self.node_execute_selected_tool)
        graph.add_node(NAME_NODE_EVALUATOR_PHASE1, self.node_evaluator_phase1)
        graph.add_node(NAME_NODE_LOAD_AGENTS, self.node_load_agents)
        graph.add_node(NAME_NODE_SELECT_AGENT, self.node_select_agent)
        graph.add_node(NAME_NODE_COMUNICATE_A2A, self.node_comunicate_a2a)
        graph.add_node(NAME_NODE_EVALUATOR_PHASE2, self.node_evaluator_phase2)
        graph.add_node(NAME_NODE_FINAL_ANSWER, self.node_final_answer)

        #graph.add_edge(NAME_NODE_LOAD_TOOLS, NAME_NODE_RETRIEVE_INFORMATION)
        graph.add_conditional_edges(NAME_NODE_LOAD_TOOLS, self.router_load_tools, {
            NAME_NODE_RETRIEVE_INFORMATION : NAME_NODE_RETRIEVE_INFORMATION,
            NAME_NODE_EVALUATOR_PHASE1 : NAME_NODE_EVALUATOR_PHASE1
        })
        graph.add_conditional_edges(NAME_NODE_RETRIEVE_INFORMATION, self.router_retriever,{
        ...
        graph.add_conditional_edges(NAME_NODE_SELECT_AGENT, self.router_select_agent, {
            NAME_NODE_COMUNICATE_A2A : NAME_NODE_COMUNICATE_A2A,
            NAME_NODE_FINAL_ANSWER : NAME_NODE_FINAL_ANSWER
        })
        graph.add_conditional_edges(NAME_NODE_COMUNICATE_A2A, self.router_comunicate_a2a, {
...
async def main2():
    ...
    agent = GraphAgent(llmController=llmController)
    ...
    result = await agent.ainvoke(role="user", log=example_log)
